...
If you are already running Ubuntu 22.04, skip to the next section.
Otherwise, follow the steps here to install Ubuntu standalone or in a dual-boot setup with your existing Windows installation.
...
We recommend you install Ubuntu 22.04.5 \url{https://www.ubuntu.com/download/desktop}. The rest of the course manuals assume that you are running Ubuntu 22.04.5. 

\textbf{NOTE} Ubuntu 24.04 is not fully supported yet. If you want to keep this OS, you can use a docker image. Read the instructions in the paparazzi folder ./docker/gazebo-classic subfolder and build the image using:

\begin{lstlisting}[style=Bash]
	cd ./docker/gazebo-classic
	docker build -t paparazzi-gazebo-classic .
\end{lstlisting}
...
\begin{enumerate}
	...
	Then the group common repository (remote) should be added on all team computers with (here names team1):
	\begin{lstlisting}[style=Bash]
	...
	\begin{lstlisting}[style=Bash]
		git checkout -t origin/!\coursebranch!
	\end{lstlisting}

	\item Now run the Paparazzi install script. There are a few options: if you don't mind installing python packages natively, simply run:

	\begin{lstlisting}[style=Bash]
		./install.sh -n
	\end{lstlisting}

	If you want the default option of installing python packages in a virtual environment, run:

	\begin{lstlisting}[style=Bash]
		./install.sh
	\end{lstlisting}

	From here on you will need to activate this environment every time you want to use Paparazzi. You can do this by running the following command in the terminal:

	\begin{lstlisting}[style=Bash]
		source ~/paparazzi/pprzEnv/bin/activate
	\end{lstlisting}

	If you prefer conda: do the following instead (and also activate the right conda environment before using Paparazzi):
	...
	Then, in the GUI, click all the buttons one at a time to install all required items. Check after each item if there were no errors during the installation. If you run into trouble, also check \url{http://wiki.paparazziuav.org/wiki/Installation}

	If this fails, you can manually install paparazzi by following the steps in \autoref{manual_install}.
	
	\item After the installation (including \texttt{make}) you can start paparazzi center:

	\begin{lstlisting}[style=Bash]
		./paparazzi
	\end{lstlisting}

	\item Select the Conf and Control Panel files that were prepared for the course:
	\begin{enumerate}
	...
	\item \textbf{Optional:} Only on Ubuntu 20.04: Build OpenCV for the Bebop if you plan to use it. Note that OpenCV is heavy and thereby slow compared to self
