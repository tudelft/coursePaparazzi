...
In case the joystick you are planning to use does not have a corresponding XML it may be necessary to create a new, custom joystick XML. Create an empty XML with your joystick name and place it in the \texttt{conf/joystick} folder (or duplicate an existing file and rename it). In the file, add the joystick tag and close it. All the code will need to be added in between.
...
Plug in your joystick and open a terminal with Ctrl+Alt+T and run \fbox{jstest-gtk}. Your joystick will likely have a number of Axis and Buttons all linked to the physical sticks and buttons on your device. Using the jstest-gtk GUI it should be easy to figure out which channel name corresponds to which action on the device. Map all the Axis and Buttons you plan to use in the XML, giving each channel index a descriptive name so it is clear to which button on the real device you are referring to. Place the code in an \texttt{<input>} block in the XML. Take a look at one of the other joystick XMLs as an example.
...
<joystick>
	<input>
		...
		...
		<button index="0" name="LeftBumper" \>
		...
	<input/>
<joystick/>
\end{lstlisting}

Now all that is left is to map Paparazzi commands (throttle, roll, mode, etc.) to an axis or button you previously defined. The values are sent as a message called \texttt{datalink}, so the code will need to be wrapped in an XML block declaring the message class.
...
Within this new block define the throttle, roll, pitch, yaw, and mode like in these examples:
...
The \textit{value} should correspond to the stick you would like to use and previously named within the \texttt{<input>} block. Note that the throttle field should usually receive positive values only, so you may need to remap the min/max values your joystick is sending using the function \texttt{Fit(\textit{input}, \textit{min}, \textit{max}, \textit{new min}, \textit{new max})}. You can map the controls in any way you want, but make sure it is possible to send a max yaw command while the throttle value stays at 0, or you will not be able to arm the drone!

If you succeed in creating a new joystick XML for a device you think others may have at home or want to use, do not hesitate to create a pull request to the tudelft/mavlabCourse2021 branch to share your work!
