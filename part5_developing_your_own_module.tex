...
\begin{mybox}
	...
	somewhere next to where the other modules are declared to ensure that it will be included during compilation. By default, a new airframe will not automatically appear in the 'A/C' dropdown menu in the Paparazzi Center. So the second step that must be taken is to add this new airframe to your desired \texttt{conf.xml}. As explained in \Cref{sec:install}, the conf used for this course is \texttt{userconf/tudelft/course\_conf.xml}. Open this file, and add a new aircraft by copy-pasting one of the other entries and replacing the name, ac\_id, and airframe field. If everything worked as expected, the next time Paparazzi is launched you should be able to compile the new aircraft!
...
\begin{mybox}
	...
	Once the module \texttt{mav\_course\_exercise} is set up to receive the optic flow results, it is a simple matter of adding and if-statement in the navigation part of the code to check whether the divergence value is higher than a threshold, for instance 0.3, in which case the navigation state can be set to OBSTACLE\_FOUND. 
\end{mybox}
