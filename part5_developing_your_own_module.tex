...
\begin{mybox}
	...
	somewhere next to where the other modules are declared to ensure that it will be included during compilation. By default, a new airframe will not automatically appear in the 'A/C' dropdown menu in the Paparazzi Center. So the second step that must be taken is to add this new airframe to your desired \texttt{conf.xml}. As explained in \Cref{sec:install}, the conf used for this course is \texttt{userconf/tudelft/course2019\_conf.xml}. Open this file, and add a new aircraft by copy-pasting one of the other entries and replacing the name, ac\_id, and airframe field. If everything worked as expected, the next time Paparazzi is launched you should be able to compile the new aircraft!
